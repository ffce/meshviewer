define(["d3"], function (d3) {
  ...
  return function (config, linkScale, sidebar, router) {
    ...
    function redraw() {
      ...
      // COLORS //
      ////////////
      var clientColor = "#1566A9"
      var unknownColor = "#D10E2A"
      var nodeFillColor = "#F2E3C6"
      var meshColor = "#00B4FF"
      var uplinkColor = "#00BB00"
      var hotspotColor = "#FA940F"
      var highlightedLinkColor = "#F2E3C6"
      ...
      if (highlightedLinks.length) {
        ctx.save()
        ctx.lineWidth = 6
        ctx.strokeStyle = highlightedLinkColor
      ...
      ////////////////////////////////
      // VISIBLE & UNSELECTED NODES //
      ////////////////////////////////
      nodes.filter(visibleNodes).forEach(function (d) {
        var size = getNodeSize(d)
        ctx.beginPath()
        ...
        ctx.strokeStyle = getNodeColor(d)
        ctx.beginPath()
        ctx.moveTo(d.x + size, d.y)
        ctx.arc(d.x, d.y, size, 0, 2 * Math.PI)
        ctx.fill()
      ...
      ////////////////////////////
      // DOT FOR SELECTED NODES //
      ////////////////////////////
      if (highlightedNodes.length)
        highlightedNodes.forEach(function (d) {
            //get node size from #links
            var size = getNodeSize(d)
          ctx.beginPath()
          ctx.moveTo(d.x + 0.5 * size, d.y)
          ctx.arc(d.x, d.y, 0.5 * size, 0, 2 * Math.PI)
          //set color based on node properties
          ctx.fillStyle = getNodeColor(d)
          ctx.fill()
          ctx.save()
