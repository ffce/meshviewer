  ...
  return function (config) {
    ...
    self.setData = function (d) {
      ...
      var totalClients = sum(d.nodes.all.filter(online).map( function (d) {
        return d.statistics.clients > 0 ? d.statistics.clients : 0
      }))
      var extraClients = sum(d.nodes.all.filter(online).map( function (d) {
        return d.statistics.clients < 0 ? 1 : 0
      }))
